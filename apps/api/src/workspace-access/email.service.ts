...
export class EmailService {
  ...
  private async deliver(message: EmailMessage): Promise<void> { if (this.sent.has(message.idempotencyKey)) return; try { await this.provider.send(message); this.sent.add(message.idempotencyKey); } catch { throw new ServiceUnavailableException('Email delivery is temporarily unavailable'); } }
