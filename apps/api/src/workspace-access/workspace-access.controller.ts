...
@Controller() export class WorkspaceAccessController {
  ...
  @Post('workspaces/:workspaceId/members/:userId/suspend') @UseGuards(JwtAuthGuard) suspendMember(@Req() r: RequestWithUser, @Param('workspaceId') workspaceId: string, @Param('userId') userId: string) { return this.service.suspendMember(r.user.sub, workspaceId, userId).then(() => ({ success: true, message: 'Member suspended', data: null })); }
  @Post('workspaces/:workspaceId/members/:userId/reactivate') @UseGuards(JwtAuthGuard) reactivateMember(@Req() r: RequestWithUser, @Param('workspaceId') workspaceId: string, @Param('userId') userId: string) { return this.service.reactivateMember(r.user.sub, workspaceId, userId).then(() => ({ success: true, message: 'Member reactivated', data: null })); }
