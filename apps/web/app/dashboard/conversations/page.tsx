...
export default function ConversationsPage() {
  ...
  async function create(): Promise<void> {
    if (!currentWorkspace) return;
    try { const conversation = await api<Conversation>(`/workspaces/${currentWorkspace.id}/ai/conversations`, { method: 'POST', body: JSON.stringify({}) }); router.push(`/dashboard/conversations/${conversation.id}`); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to create conversation.'); }
  }
  ...
  return <section className="conversation-page">
    ...
    {!workspaceLoading && currentWorkspace && <form className="conversation-search" onSubmit={submitSearch}><label htmlFor="conversation-search">Search conversations</label><div><input id="conversation-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by title" maxLength={100} /><button className="button button-small" type="submit">Search</button></div></form>}
