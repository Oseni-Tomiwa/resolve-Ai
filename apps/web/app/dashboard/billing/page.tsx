...
export default function BillingPage() {
  ...
  return <section className="billing-page">
    <div className="billing-heading"><div><p className="eyebrow">Workspace billing</p><h2>Plans that scale with your support team</h2><p>Usage is measured per workspace and resets at the start of each month.</p></div><div className="billing-heading-actions"><span className="billing-plan-badge">{currentPlanName}</span>{billing.subscription.provider === 'stripe' && <button className="button button-ghost button-small" type="button" onClick={() => void openPortal()}>Manage billing</button>}</div></div>
    ...
    <div className="invoice-placeholder"><div><p className="eyebrow">Invoices</p><h3>Invoice history</h3><p>Invoices and receipts are managed securely in the Stripe customer portal.</p></div>{billing.subscription.provider === 'stripe' ? <button className="button button-ghost button-small" type="button" onClick={() => void openPortal()}>View invoices</button> : <span>Stripe billing is not active</span>}</div>
  </section>;
}
