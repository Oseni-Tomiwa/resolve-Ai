...
export async function middleware(request: NextRequest) {
  ...
  if ((pathname.startsWith('/dashboard') || pathname === '/onboarding') && !hasCookie && !session.authenticated) { const url = request.nextUrl.clone(); url.pathname = '/login'; url.searchParams.set('redirect', pathname); return NextResponse.redirect(url); }
